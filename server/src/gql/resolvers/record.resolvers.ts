...
import { PAGE_TYPES } from './print.model'
...
const createdAt = _.compose(_.prop('createdAt'), _.prop('doc') as any)

const sortByCreated = _.descend(createdAt)
...
export const records = async (parent: any, args: any) => {
  ...
  const payload = limited.map(async (row: any) => {
    const doc = row
